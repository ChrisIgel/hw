...
begin
    ...
    begin
        if (Gear^.AmmoType = amDEagle) or (Gear^.AmmoType = amMinigun) then
            ...
            dmg:= 10 + hwRound(int2hwFloat((Gear^.Timer div 6) * (Gear^.Timer div 6) * Gear^.Boom) * _0_5);
...
begin
    ...
    if (Gear^.Timer mod 40) = 0 then
        begin
        rx := rndSign(getRandomf * _0_15);
        ry := rndSign(getRandomf * _0_15);
...
begin
    ...
    graves := GearsNear(Gear^.X, Gear^.Y, gtGrave, cResurrectorDist + (CurrentHedgehog^.ResurrectorHealth div 50));
...
begin
    AllInactive := false;
    CurrentHedgehog^.ResurrectorHealth := CurrentHedgehog^.Gear^.Health;
    graves := GearsNear(Gear^.X, Gear^.Y, gtGrave, cResurrectorDist + (CurrentHedgehog^.ResurrectorHealth div 50));
