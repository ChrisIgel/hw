...
case Kind of
          gtFlame: Gear^.Boom := 4;  // some additional expl in there are x3, x4 this
       gtHedgehog: Gear^.Boom := 50;
           gtMine: Gear^.Boom := 50;
           gtCase: Gear^.Boom := 25;
        gtAirMine: Gear^.Boom := 80;
...
gtSniperRifleShot: Gear^.Boom := 600000;
    ...
    gtSineGunShot: Gear^.Boom := 200;
          gtSMine: Gear^.Boom := 80;
    gtSnowball: Gear^.Boom := 500000; // arbitrary scaling for the shove
         gtHammer: if cDamageModifier > _1 then // scale it based on cDamageModifier?
                         Gear^.Boom := 2
                    else Gear^.Boom := 3;
    gtPoisonCloud: Gear^.Boom := 80;
          gtKnife: Gear^.Boom := 60000; // arbitrary scaling factor since impact-based
  ...
  gtPickHammer: begin
   ...
   gtHammerHit: begin
     ...
     gtAirMine: begin
                ...
                gear^.Friction:= _0_995;
                gear^.Density:= _1;
                gear^.Angle:= 400; // Radius at which air bombs will start "seeking". $FFFFFFFF = unlimited. check is skipped.
